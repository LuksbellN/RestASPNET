...
namespace RestASPNET.Hypermedia
{
    public abstract class ContentResponseEnricher<T> : IResponseEnricher where T : ISupportsHypermedia
    {
        ...
        protected abstract Task EnricheModel(T content, IUrlHelper urlHelper);
        ...
        public async Task Enrich(ResultExecutingContext response)
        {
            ...
            if (response.Result is OkObjectResult okObjectResult)
            {
                ...
                {
                    await EnricheModel(model, urlHelper);
                }
                else if (okObjectResult.Value is List<T> collection)
                {
                    ...
                    {
                        EnricheModel(element, urlHelper);
                    });
                } 
                else if(okObjectResult.Value is PagedSearchVO<T> pagedSearch)
                {
                    Parallel.ForEach(pagedSearch.Data.ToList(), element =>
                    {
                        EnricheModel(element, urlHelper);
                    });
                }
